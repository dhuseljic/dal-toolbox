...
def train_one_epoch(model, dataloaders, criterion, optimizer, device, n_epochs, unsup_warmup, 
    ...
    for (x_lb, y_lb), (x_ulb_weak, y_ulb_weak), (x_ulb_strong, y_ulb_strong) in zip(dataloaders['train_sup'], dataloaders['train_unsup_weak'], dataloaders['train_unsup_strong']):
        x_lb, y_lb = x_lb.to(device), y_lb.to(device)
        x_ulb_weak, y_ulb_weak = x_ulb_weak.to(device), y_ulb_weak.to(device)
        x_ulb_strong, y_ulb_strong = x_ulb_strong.to(device), y_ulb_strong.to(device)
        ...
        unsup_loss = consistency_loss(logits_ulb_strong,
                                          torch.softmax(logits_ulb_weak.detach(), dim=-1),
                                          'mse')
