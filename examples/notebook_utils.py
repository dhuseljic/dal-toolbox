...
def plot_dset(X_l, y_l, X_u=None, y_u=None, ax=None):
    if ax:
        plt.sca(ax)
    if X_u != None:
        plt.scatter(X_l[:, 0], X_l[:, 1], c=y_l, s=100, edgecolors='black', linewidths=3, label='labeled', zorder=5)
        plt.scatter(X_u[:, 0], X_u[:, 1], c=y_u, s=10, alpha=0.5, label='unlabeled')
        plt.legend()
    else:
        plt.scatter(X_l[:, 0], X_l[:, 1], c=y_l, s=50, zorder=5)
    plt.show()
...
def plot_contour(model, X_l, y_l, X_u=None, y_u=None, ax=None, x_domain=3, y_domain=3, forward_mode='single'):
    ...
    CS = plt.contourf(xx.numpy(), yy.numpy(), zz.numpy(), alpha=.8, zorder=-1, levels=np.linspace(0, 1, 6), origin=origin)
