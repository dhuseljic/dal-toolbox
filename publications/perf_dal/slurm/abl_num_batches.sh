...
selection_strategies=(\[random\] \[random,typiclust,dropquery,bait,typiclass,dropqueryclass,loss,margin,badge,coreset,alfamix\])
...
sel_strats=${selection_strategies[$index / 2 % 3]}
var_sss=${vary_subset_size[$index / 2 % 3]}

n_bat=${num_batches[$index / 6 % 3]}

random_seed=${random_seeds[$index / 18]}
