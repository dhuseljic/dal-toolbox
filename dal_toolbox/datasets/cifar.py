...
#     def __init__(self, root, imbalance_ratio=0.01, imb_type='exp', train=True, transform=None, target_transform=None, download=False):
#         # 1. Initialize the standard CIFAR10 (loads all data)
#         super().__init__(root, train=train, transform=transform, target_transform=target_transform, download=download)

#         # 2. Use the Wrapper purely to calculate the indices (Logic reuse)
#         # We pass 'self' as the dataset. The wrapper calculates which indices to keep.
#         # Note: We convert ratio 0.01 -> 100 for the wrapper if your wrapper expects >1,
#         # or keep it if your wrapper handles <1. Assuming your wrapper expects a ratio > 1 (e.g. 100):
#         calc_ratio = 1.0 / imbalance_ratio if imbalance_ratio < 1 else imbalance_ratio

#         # We create a temporary wrapper just to get the 'indices' logic
#         # We set seed=42 to ensure consistent data splits across runs
#         temp_wrapper = LongTailedDatasetWrapper(self, imbalance_ratio=calc_ratio, imb_type=imb_type, seed=42)

#         # 3. Slice the data in-place
#         # CIFAR10 stores images in self.data (numpy) and labels in self.targets (list)
#         kept_indices = temp_wrapper.indices

#         self.data = self.data[kept_indices]
#         self.targets = np.array(self.targets)[kept_indices].tolist() # Convert to array to slice, then back to list
