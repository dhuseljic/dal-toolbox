...
selection_strategies=(\[random,alfamix\] \[random,badge\] \[random,bait\] \[random,coreset\] \[random,dropquery\] \[random,dropqueryclass\] \[random,margin\] \[random,random\] \[random,typiclass\] \[random,typiclust\])
...
sel_strats=${selection_strategies[$index / 2 % 10]}
