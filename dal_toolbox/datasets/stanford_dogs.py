...
class StanfordDogs(BaseData):
    ...
    def download_datasets(self):
        _StanfordDogs(self.dataset_path, split="train")
        _StanfordDogs(self.dataset_path, split="test")

    @property
    def full_train_dataset(self):
        return _StanfordDogs(self.dataset_path, split="train", transform=self.train_transform)

    @property
    def full_train_dataset_eval_transforms(self):
        return _StanfordDogs(self.dataset_path, split="train", transform=self.eval_transform)

    @property
    def full_train_dataset_query_transforms(self):
        return _StanfordDogs(self.dataset_path, split="train", transform=self.query_transform)
