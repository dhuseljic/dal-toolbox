...
This folder contains a minimal example of how to use the different tools provided by our DAL-Toolbox. The primary toolsets contained in the toolbox are:
...
Recommended imports and usage are shown in [active_learning.py](./active_learning.py). In addition, we provide a minimal [slurm script](./slurm_example.sh) that is ready to be run and extended once you replace the placeholders with your individual settings.
